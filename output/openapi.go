...
func (o *OpenAPIFileContext) GetOpHappyResponse(pkg string, op *openapi3.Operation) OpResponse {
		...
		for _, mimeType := range supportedResponseContentTypes {
			...
			if mediaType != nil {
				...
				if r.Value.Headers != nil {
					return OpResponse{Key: key, MimeType: mime, MediaType: mediaType, GoType: goType, Headers: sortedHeaderKeys(r.Value.Headers)}
				}
	...
	for _, key := range happyKeys {
		...
		if len(r.Value.Headers) > 0 {
			return OpResponse{Key: key, MimeType: "", MediaType: nil, GoType: "", Headers: sortedHeaderKeys(r.Value.Headers)}
		}
	}
...
func sortedHeaderKeys(in openapi3.Headers) []string {
