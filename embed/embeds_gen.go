// Code generated by foji 0.3, template: foji/embed.go.tpl; DO NOT EDIT.
...
func List() []string {
	return []string{
		...
		"foji/enum.go.tpl",
		"foji/fields.go.tpl",
		"foji/openAPIStub.yaml.tpl",
		"foji/openapiComponent.go.tpl",
		"foji/sqlRepo.go.tpl",
...
func Get(filename string) (string, error) {
	...
	case "foji/openAPIStub.yaml.tpl":
		return FojiSlashOpenApistubDotYamlDotTpl, nil
	case "foji/openapiComponent.go.tpl":
		return FojiSlashOpenapiComponentDotGoDotTpl, nil
...
var FojiSlashOpenApistubDotYamlDotTplBytes = []byte(FojiSlashOpenApistubDotYamlDotTpl)

const FojiSlashOpenApistubDotYamlDotTpl = `openapi: "3.0.0"
info:
...
var FojiSlashOpenapiComponentDotGoDotTplBytes = []byte(FojiSlashOpenapiComponentDotGoDotTpl)

const FojiSlashOpenapiComponentDotGoDotTpl = `// Code generated by foji {{ version }}, template: {{ templateFile }}; DO NOT EDIT.
...
{{- range $key, $schema := .Value.Properties }}
    ...
    {{ pascal $key }}  {{print $typeName (pascal $key) "Enum"}} ` + "`" + `json:"{{$key}},omitempty"` + "`" + ` {{ if not (empty .Value.Description) }}// {{ .Value.Description }}{{ end }}
    {{- else }}
    {{ pascal $key }} {{ $.GetType $pkgName $key $schema }} ` + "`" + `json:"{{$key}},omitempty"` + "`" + ` {{ if not (empty .Value.Description) }}// {{ .Value.Description }}{{ end }}
    {{- end }}
{{- end }}
}
{{- range $key, $schema := .Value.Properties }}
...
{{- range $key, $schema := .Value.Properties }}
    {{- if in $schema.Value.Type "number" "integer" }}
        {{- $fieldType := $.GetType $pkgName $key $schema }}
...
{{- range .Result.Params.ByOrdinal }}
    {{ pascal .Name }} {{ $.GetType . $pkgName }}  ` + "`" + `json:"{{ .Name }},omitempty"` + "`" + ` // {{ .Type }}
{{- end }}
}
{{- end }}
...
// {{.Comment}}
func (r Repo) {{ .Name }}(ctx context.Context{{if gt (len .Params) 0}},{{end}} {{ $.Parameterize .Params.ByOrdinal "%s %s" $pkgName }}) ({{ if .IsType "query" }}[]{{ end }}*{{$.GetType .Result.TypeParam $pkgName}}, error) {
    query := ` + "`" + `{{ .SQL }}` + "`" + `
...
{{/*{{- $mutableCols := .Table.MutableColumns.ByOrdinal.Names }}*/}}
{{- $mutableCols := (.Table.Columns.Filter .Table.PrimaryKeys.Paths).ByOrdinal.Names }}
{{/*{{- $mutableFields := .Table.MutableColumns.ByOrdinal.Names.Sprintf "row.%s"}}*/}}
...
func scan{{$goName}}(rr pgx.Rows) ([]*{{.Params.RootPkg}}.{{$goName}}, error) {
	var result []*{{.Params.RootPkg}}.{{$goName}}
	for rr.Next() {
		row := {{.Params.RootPkg}}.{{$goName}}{}
...
func scanOne{{$goName}}(rr pgx.Row) (*{{.Params.RootPkg}}.{{$goName}}, error) {
	row := {{.Params.RootPkg}}.{{$goName}}{}
	err := rr.Scan({{ csv $scanFields }})
	if err != nil {
...
func (r Repo) All{{$goName}}(ctx context.Context) ([]*{{.Params.RootPkg}}.{{$goName}}, error) {
...
func (r Repo) Count{{$goName}}(ctx context.Context, where {{.Params.RootPkg}}.WhereClause) (int, error) {
	idx := 1
	query := ` + "`" + `SELECT
		count(*) as count
		FROM {{$schema}}.{{$table}}
...
func (r Repo) Select{{$goName}}(ctx context.Context, where {{.Params.RootPkg}}.WhereClause) ([]*{{.Params.RootPkg}}.{{$goName}}, error) {
	idx := 1
...
func (r Repo) SelectOrder{{$goName}}(ctx context.Context, where {{.Params.RootPkg}}.WhereClause, orderBy {{.Params.RootPkg}}.OrderByClause) ([]*{{.Params.RootPkg}}.{{$goName}}, error) {
	idx := 1
	query := querySelect{{$goName}} + " WHERE " + where.String(&idx)
{{- if $hasSoftDeletes -}}
	+ ` + "`" + ` AND deleted_at is NULL ` + "`" + `
...
func (r Repo) First{{$goName}}(ctx context.Context, where {{.Params.RootPkg}}.WhereClause, orderBy {{.Params.RootPkg}}.OrderByClause) (*{{.Params.RootPkg}}.{{$goName}}, error) {
	idx := 1
	query := querySelect{{$goName}} + " WHERE " + where.String(&idx)
{{- if $hasSoftDeletes -}}
	+ ` + "`" + ` AND deleted_at is NULL ` + "`" + `
...
func (r Repo) Insert{{$goName}}(ctx context.Context, row *{{.Params.RootPkg}}.{{$goName}}) error {
...
func (r Repo) Update{{$goName}}(ctx context.Context, row *{{.Params.RootPkg}}.{{$goName}}) error {
...
func (r Repo) Set{{$goName}}(ctx context.Context, set {{.Params.RootPkg}}.Where, where {{.Params.RootPkg}}.WhereClause) (int64, error) {
	idx := 2
...
func (r Repo) DeleteWhere{{$goName}}(ctx context.Context, where {{.Params.RootPkg}}.WhereClause) (int64, error) {
	idx := 1
{{ if $hasSoftDeletes}}
	query := ` + "`" + `UPDATE {{$schema}}.{{$table}}
		SET deleted_at = now()
...
// Returns the number of items deleted.
func (r Repo) DeleteWherePermanent{{$goName}}(ctx context.Context, where {{.Params.RootPkg}}.WhereClause) (int64, error) {
	idx := 1
	query := ` + "`" + `DELETE FROM {{$schema}}.{{$table}}
		WHERE ` + "`" + ` + where.String(&idx)
...
// UndeleteWhere{{$goName}} undeletes the Row from the database.
func (r Repo) UndeleteWhere{{$goName}}(ctx context.Context, where {{.Params.RootPkg}}.WhereClause) (int64, error) {
	idx := 1
	query := ` + "`" + `UPDATE {{$schema}}.{{$table}}
		SET deleted_at = null
...
func (r Repo) {{ $FuncName }}(ctx context.Context, {{ $.Parameterize .Columns "%s %s" $pkgName }}) ({{ if not .IsUnique }}[]{{ end }}*{{$.Params.RootPkg}}.{{$goName}}, error) {
...
const InitDotYaml = `db:
  connection: "host=localhost dbname=MyProject sslmode=disable"
  filter:
    - "*.schema_migrations"
sql:
  files:
